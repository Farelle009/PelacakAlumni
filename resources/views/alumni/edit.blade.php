...
@section('page-description', 'Perbarui data alumni yang sudah tersimpan')
...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6">
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            {{-- Section header --}}
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-slate-900">{{ $alumni->nama_lengkap }}</h3>
                <p class="mt-1 text-sm text-slate-500">Perbarui data di bawah ini lalu tekan Update Alumni.</p>
            </div>
                ...
                >
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="program_studi"
                                id="program_studi"
                                value="{{ old('program_studi', $alumni->program_studi) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            >
                                @foreach ($statusOptions as $option)
                                    <option value="{{ $option }}"
                                        @selected(old('status_pelacakan', $alumni->status_pelacakan) === $option)>
                                        {{ $option }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        >
                            ...
                            <span x-text="loading ? 'Menyimpan…' : 'Update Alumni'"></span>
                        </button>

                        <a
                            href="{{ route('alumni.show', $alumni) }}"
        ...
        {{-- ── Danger Zone ───────────────────────────────────────────────────── --}}
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-rose-200">
            <h3 class="text-base font-semibold text-rose-700">Zona Berbahaya</h3>
            <p class="mt-1 text-sm text-slate-500">
                Hapus permanen data alumni beserta seluruh hasil tracking yang terkait. Tindakan ini tidak dapat dibatalkan.
            </p>

            <div class="mt-4" x-data="{ loading: false }">
                <form
                    action="{{ route('alumni.destroy', $alumni) }}"
                    method="POST"
                    x-ref="deleteForm"
                >
                    @csrf
                    ...
                    <button
                        ...
                        :disabled="loading"
                        @click="
                            if (confirm('Yakin ingin menghapus {{ addslashes($alumni->nama_lengkap) }}? Data tidak bisa dipulihkan.')) {
                                loading = true;
                                $refs.deleteForm.submit();
                            }
                        "
                        class="inline-flex items-center gap-2 rounded-xl border border-rose-300 bg-rose-50 px-5 py-3 text-sm font-medium text-rose-700 transition hover:bg-rose-100 disabled:opacity-60"
                    >
                        <svg x-show="loading" class="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                        </svg>
                        <span x-text="loading ? 'Menghapus…' : 'Hapus Alumni Ini'"></span>
                    </button>
                </form>
            </div>
        </div>
